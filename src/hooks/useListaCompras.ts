...
export function useListaCompras() {
  ...
  const updatePreco = async (refeicao: string, itemId: string, novoPreco: number) => {
    await atualizarItem(refeicao, itemId, { preco: novoPreco });
  ...
  const adicionarItemSugerido = async (refeicao: string, nomeItem: string) => {
    const novoItem = {
      ...
      nome: nomeItem,
      quantidade: "1 unit",
      preco: 0, // User will set the price
      comprado: false
    };
    await adicionarItem(refeicao, novoItem);
  };
